...
export class PriceService {
  ...
  async do(
    ...
    console.time('timer_price');
    console.log('Iniciando atualização de preço');
    ...
    for await (const plugin of plugins) {
      try {
        ...
        if (content.active == '1')
          if (content.ablePrice == '1') await this.update(plugin, payloadDto);
      } catch (error) {
        console.timeEnd('timer_price');
        console.log(error);
      }
    }
    console.timeEnd('timer_price');
  ...
  async update(plugin: any, payloadDto: PayloadDto): Promise<any> {
    try {
      ...
      for await (const i of arr) {
        ...
        if (params[0].content.ablePrice == '0') {
          console.log(`\x1b[41m\x1b[37m CARGA DE PREÇO DESATIVADO \x1b[0m`);
          break;
        }
            ...
            if (stock) {
              if (stock.price != item.price) {
                ...
                await getConnection()
                  ...
                  .execute();
                console.log(
                  `Preço \x1b[36m atualizado \x1b[0m`,
                  item.price,
                  product.name,
                );
              } else {
                console.log(
                  `Preço \x1b[42m\x1b[31m inalterado \x1b[0m`,
  ...
  async _load(
    ...
    const url = content.url + content.url_price;
