...
export class ClassificationController {
  ...
  // @ApiResponse({ status: 400, description: 'Bad Request.' })
  // @ApiResponse({ status: 403, description: 'Forbidden.' })
  // async allGet(@Req() req: Request, @Res() res: Response) {
  //   console.log(`Classificador \x1b[32m AWS [${req.ip}] \x1b[0m`);
  //   const ret = await this.classificationService.do(undefined);
  //   if (ret.success) {
  //     return res.status(HttpStatus.OK).json(ret);
  //   } else {
  //     return res.status(HttpStatus.BAD_REQUEST).json(ret);
  ...
  // @ApiResponse({ status: 400, description: 'Bad Request.' })
  // @ApiResponse({ status: 403, description: 'Forbidden.' })
  // async byIdGet(@Req() req: Request, @Res() res: Response) {
  //   console.log(`Classificador \x1b[32m MANUAL [${req.ip}] \x1b[0m`);
  //   const ret = await this.classificationService.do(req.params.id, req.ip);
  //   if (ret.success) {
  //     return res.status(HttpStatus.OK).json(ret);
  //   } else {
  //     return res.status(HttpStatus.BAD_REQUEST).json(ret);
